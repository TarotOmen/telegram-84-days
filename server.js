...
/*
  Можно указать числовой ID товара вручную
  через Render Environment Variables.

  Если не указан — сервер попробует определить
  его при запуске через Tribute API.
*/
...
async function resolveTributeProductId() {
  ...
  if (!TRIBUTE_API_KEY) {
    console.log(
      "TRIBUTE_API_KEY is missing. Tribute product ID cannot be resolved."
    );
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(
        `Tribute API returned HTTP ${response.status}`
      );
    }
    ...
    if (!target?.id) {
      console.log(
        "84 Days Tribute product was not found by webLink."
      );

      console.log(
        `Looking for: ${TRIBUTE_PRODUCT_URL}`
      );
...
/* =========================
   FORWARD NON-84 EVENTS TO TAROT
========================= */
...
async function forwardTributeToTarot(
  ...
  if (!response.ok) {
    throw new Error(
      `Tarot webhook returned HTTP ${response.status}: ${text.slice(
        0,
        300
      )}`
    );
...
async function processTributeEvent(
  ...
  if (
    ...
    if (fresh) {
      ...
      console.log(
        `Duplicate refund ignored: ${purchaseId}`
      );
    }
...
/*
  ...
  Tribute получает 200 OK СРАЗУ.

  Мы НЕ ждём:
  - PostgreSQL
  - Tarot
  - Tribute API
  - любые другие внешние запросы

  Поэтому webhook не должен
  упираться в 10 секунд.
*/
...
app.post(
  ...
  (req, res) => {
    ...
    // Сначала сразу отвечаем Tribute.
    res.status(200).json({
      ok: true
    });
...
async function start() {
  try {
    ...
    /*
      Определяем Tribute product_id
      ДО запуска webhook.

      То есть во время webhook
      внешний запрос к Tribute
